...
int main (void) {
  ...
  while(side != 0) {	// while(1) is an infinite loop / while(side != 0) is a breaking out condition (if side = 0)
    printf("\nGive shape side dimension (Type zero to quit): ");  // Prompt user
    scanf("%ld", &side);         // Get user input (unsigned long type)
    area = Calc_Area(side);
    if (area != 0) {
      printf("\nArea of a square shape of side %ld is %ld\n",  side, area);
    } else {
...
unsigned long Calc_Area(unsigned long side) {
  ...
  if (side <= 25) {
    ...
    error 	= error + 1;  			// increment global error count 
  } 
